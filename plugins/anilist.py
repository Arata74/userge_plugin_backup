...
ANIME_TEMPLATE = """**{mal_link}**

✽ **ENGLISH NAME:** `{english}`
✽ **ANILIST ID:** `{idm}`
✽ **MAL ID:** `{idmal}`
✽ **SOURCE:** `{source}`
✽ **COUNTRY:** `{c_flag} {country}`
✽ **TYPE:** `{formats}`
✽ **GENRES:** `{genre}`
✽ **STUDIOS:** {studios}
✽ **START ON:** `{start_date}`
...
📖 [SYNOPSIS & CHARACTER]({synopsis_link})

#{formats} #{status} #{season}
#{season}{ser} #{source}"""
...
def post_to_tp(a_title, content):
    ...
    auth_name = "Anime Dynasty"
    post_client.create_api_token(auth_name)
    post_page = post_client.post(
        title=a_title,
        author=auth_name,
        author_url="https://t.me/AnimeDynastyEN",
        text=content
    )
...
async def anim_arch(message: Message):
    ...
    mal_link = f"[{romaji}](https://myanimelist.net/anime/{idmal})"
    report_link = "[REPORT](https://surveyheart.com/form/61fab37c5eebce35375dedce)"
    schedule_link = "https://telegra.ph/WEEKLY-SCHEDULE-GMT824H-02-13"
    anime_link = "[ANIME PROGRESS](https://t.me/AnimeProgress)"
    adult = data.get('isAdult')
    trailer_link = "NOT FOUND"
    ...
    title_img2 = coverImg or bannerImg
    title_img = f"https://img.anili.st/media/{idm}" or bannerImg
    ...
    if '-wp' in message.flags:
        finals_ = f"[\u200b]({title_img2}) {finals_}"
        await message.edit(finals_)
        return
    await message.reply_photo(title_img2, caption=finals_)
